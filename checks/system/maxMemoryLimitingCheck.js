if (xmlPolicy) {
    ...
    if (res.length > 0) {
    	...
    	for (var issue of insecureMemorySet)
	        ...
	        "description": `The policy is setting a memory request limit in bytes over <code>256 MiB</code>. This policy is used to determine the maximum amount of memory space in bytes that is permitted for allocation requests. Some image processing algorithms (e.g. wavelet transform) might require a substantial amount of memory to complete. ImageMagick maintains a separate memory pool for these large resource requests and as of 7.0.6-1 permits you to set a maximum request limit. If the limit is exceeded, an exception is thrown and the processing stops. This limit can be set by the maximum memory request by policy. Note that depending on your usage or environment, these values could still be acceptable.`
	    	});
