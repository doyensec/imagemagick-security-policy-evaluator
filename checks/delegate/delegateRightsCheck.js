if (xmlPolicy) {
    ...
    if (res.length > 0) { // there's at least a "delegate" with rights & pattern
        ...
        if (insecureDelegatesNotLimited.length === 0) // all patterns are not limited first
            ...
            "description": `The current delegate policy is not denying first the permissions for all delegates. To avoid a denylist approach, you should at first exclude all the delegates setting <code>rights</code> to <code>none</code> for all patterns (using <code>*</code>) at the beginning of the policy, and then allow only a safe subset of delegates.`
            });
