if (xmlPolicy) {
    ...
    if (res.length > 0) {
	    ...
	    return {
	        ...
	        "description": `The policy is not setting any list length limit. The default limit is <code>32</code>, but it is adjusted relative to the available resources on the machine if this information is available. This policy is used to determine the maximum number of images in a sequence space that can be processed for the final image. Note that depending on your usage or environment, not setting a value could still be acceptable, even if discouraged.`
	    }
	}
